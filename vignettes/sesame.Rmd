...
# Lift over across platforms and imputation
...
## Project probe IDs to a different platform
...
## Project beta values with and without imputation

```{r}
betas = openSesame(sesameDataGet("EPICv2.8.SigDF")[[1]])
betas_epic = mLiftOver(betas, "EPIC", impute=FALSE)
length(betas_epic)
sum(is.na(betas_epic))

betas_epic = mLiftOver(betas, "EPIC", impute=TRUE)
length(betas_epic)
sum(is.na(betas_epic))
...
## harmonize raw signal intensities stored as a SigDF
